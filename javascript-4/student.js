...
const faveColors = ["green", "purple", "yellow"];
...
const me = {
    firstName: "Lauren",
    superHeroName: "Super Coder",
    homeTown: "SLC",
    superPowers: ["HTML", "CSS", "JS"],
    superPowerXP: randomNum => Math.floor(Math.random() * 100) + 1,
    profileImage: image => `https://randomuser.me/api/portraits/med/lego/${Math.floor(Math.random() * 10) + 1 }.jpg`
}
...
function setColor(arr){
    let trimmedColors = arr.splice(0, 3);
    for(let i=0; i < arr.length; i++){
        if(i === 'blue'){
            trimmedColors.splice(i, 1, "#4D4DFF")
        }
    }
    background(trimmedColors[0], trimmedColors[1], trimmedColors[2])
}
